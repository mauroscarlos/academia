...
# --- 2. TREINAR AGORA ---
    ...
    meus_treinos_df = pd.read_sql(text("SELECT DISTINCT treino_nome FROM fichas_treino WHERE usuario_id = :u"), engine, params={"u": st.session_state.user_id})
    if meus_treinos_df.empty:
        st.warning("Nenhuma ficha cadastrada.")
    else:
        t_sel = st.selectbox("Escolha o Treino:", meus_treinos_df['treino_nome'].tolist())
...
# --- 5. GESTÃO DE USUÁRIOS ---
    ...
    with st.form("u_f"):
        ...
        if st.form_submit_button("Cadastrar"):
            ...
            enviar_email_cadastro(nome, email, u_l, senha); st.success("Pronto!")
